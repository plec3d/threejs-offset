...
const applyOffset = async (meshToOffset, offset) => {
  ...
  const result2 = exporter.parse( meshOffset, { binary: false } );
  const blob = new Blob([result2], { type: 'text/plain' });
...
const processGeometry = (geometry, offset) => {
  ...
  const newMaterial = new THREE.PointsMaterial({ color: 0xff0000, size: 0.06 });
  const newMesh = new THREE.Points(newGeometry, newMaterial);
  const edgeGeo = new THREE.EdgesGeometry(geometry,5);
  const line = new THREE.LineSegments(
          edgeGeo,
          new THREE.LineBasicMaterial({
              color: new THREE.Color('white')
          }));
  removeAddOffset(line);
  newMesh.name = "offset";
